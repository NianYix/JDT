"""Test generation use cases (async AI job)."""
...
class TestGenerationService:
    def __init__(
        ...
        *,
        ...
        self._test_generations = TestGenerationRepository(db)
    ...
    def _build_context(
        ...
        *,
        ...
        sections: list[str] = [f"--- Test Target ---\n{target_description}"]
    ...
    def create(
        ...
        row = self._test_generations.create(
        ...
        def _job() -> None:
            TestGenerationService._execute(record_id, project_id, provider)

        schedule_ai_job(background_tasks, _job)
        return TestGenerationPublic.model_validate(row)
    ...
    def _execute(
        ...
        def _run(db: Session) -> None:
            ...
            try:
                ...
                llm_context = TestGenerationService._build_context(
                    db,
                    project_id,
                ...
                result = provider.generate_tests(llm_context)
                row.status = AiJobStatus.SUCCEEDED
                row.result_json = result.model_dump()
                row.model_name = model_name
                row.error_message = None
            except Exception as exc:  # noqa: BLE001
                logger.exception("Test generation LLM failed: %s", exc)
    ...
    def list_for_project(
        ...
        *,
        ...
        items, total = self._test_generations.list_by_project(
            project_id,
            page=page,
            page_size=page_size,
        )
        return PageResponse(
            items=[TestGenerationPublic.model_validate(item) for item in items],
            total=total,
            page=page,
            page_size=page_size,
        )
    ...
    def get_for_project(
        ...
        require_owned_project(self._db, owner, project_id)
        row = self._test_generations.get_by_id_for_project(generation_id, project_id)
        if row is None:
            raise AppError(
                "Test generation not found",
                code="test_generation_not_found",
                status_code=404,
            )
        return TestGenerationPublic.model_validate(row)
